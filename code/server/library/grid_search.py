...
class grid_search(Model,DataProcessing):
    ...
    def __template(_self):
        ...
        _self.grid_search=GridSearchCV(estimator=_self.model,param_grid=_self.param_grid,cv=10)
    ...
    def get_best_score_test(_self):
        return _self.grid_search.score(_self.X_test,_self.y_test)
        

    def get_best_params(_self):
        return json.dumps(_self.grid_search.best_params_)
    
    def get_best_score_train(_self):
        return _self.grid_search.best_score_
    
    def get_best_estimator(_self):
        return _self.grid_search.best_estimator_
    
    def get_grid_search(_self):
        return _self.grid_search
    
    def get_results(_self):
        return _self.results
    
    def __compute_results(_self):
        _self.results['Best Score']=_self.grid_search.best_score_
        _self.results['Best Parameters']=_self.grid_search.best_params_
        _self.results['Best Estimator']=_self.grid_search.best_estimator_
        _self.results['Best Index']=_self.grid_search.best_index_
        _self.results['Scorer']=_self.grid_search.scorer_
        _self.results['CV Results']=_self.grid_search.cv_results_
        _self.results['Grid Search']=_self.grid_search
